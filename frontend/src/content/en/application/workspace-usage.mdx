...
- **Header total**: the persisted thread-level total from the backend. While the
  ...
  in-flight usage for that unfinished response. When the provider reports
  prompt-cache hits, this same header counter and dropdown show cache-read
  tokens, uncached input, and the cache hit rate.
- **Per-turn / debug usage**: usage derived from the assistant messages that are
  currently visible in the conversation view. These summaries keep their
  existing token layout and do not duplicate the cache breakdown.
