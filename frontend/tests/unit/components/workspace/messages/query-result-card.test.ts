...
it("renders validated SQL and successful rows", () => {
  const artifact: QuerySqlResultArtifact = {
    ...
    validation: {
      valid: true,
      executable_sql: "SELECT region, SUM(order_amount) AS total FROM orders GROUP BY region LIMIT 500",
  ...
  const html = renderToStaticMarkup(createElement(QueryResultCard, { artifact }));
