...
- **实验性 DataAgent 运行层**：`backend/packages/harness/deerflow-dev/agents/data_agent` 通过 `build_data_agent()` / `make_data_agent()` 调用 `deerflow.agents.factory.create_deerflow_agent(...)` 重新创建图，并插入 `QueryContextMiddleware` 与 `DataAgentOrchestrationMiddleware`；该入口当前未注册 Gateway 路由。
- **DataAgent 状态扩展**：`DataAgentState` 继承 `ThreadState`，新增 `data_agent_stage`、`data_query_context`、`data_retrieval_context`、`data_generated_sql`、`data_sql_validation`、`data_sql_execution`、`data_last_successful_sql_execution` 和 `data_chart_spec`。
- **DataAgent 专用工具**：`build_data_agent_runtime_tools()` 注册 `data_validate_sql`、`data_execute_sql`、`data_build_chart_spec`。前两者通过 SQL 摘要绑定，禁止执行未经过最近一次校验的 SQL。
