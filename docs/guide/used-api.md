...
- **TableRAG 登记**：`TableRagStageMiddleware` 只拦截只读 `tablerag_retrieve`、`tablerag_raw_retrieve`、`tablerag_search_*` 和 `tablerag_expand_join_graph`。成功结果生成类型化 ref 和 retrieval digest；首次失败/空结果进入 `needs_refinement`，同轮补充检索失败只记录安全错误并保留最近成功 Evidence。
...
- **标签工具**：`publish_query_labels` 只由 DataAgent service ability 提供，不在全局 `BUILTIN_TOOLS`。参数为 `intent`、完整 `labels`、可选 `summary/confidence/ambiguities`；数据库标签必须引用当前 registry 中的 `evidence_refs`。
- **标签 artifact**：`kind=data_query_labels`、`version=1`，携带 `service_name`、`snapshot_id`、`data_source_id`、labels、ambiguities、confidence 和 approval。ToolMessage/custom event 仅用于历史与 UI，不是执行授权。
- **确认协议**：需要确认时复用 human-input v1，request 使用 `source=ask_clarification` 与 `request_id=data-query:<digest>`。响应仍是现有隐藏 HumanMessage；后端从 checkpoint 查 pending snapshot，并支持 `execute`、`sql_only`、`cancel` 和自由文本修改。
...
- TableRAG MCP：`extensions_config.example.json -> mcpServers.tablerag`。
