...
- 新增实验性包：`backend/packages/harness/deerflow-dev/agents/data_agent/`。
...
- 新增 `QueryContextMiddleware`：
  - 黑话/别名归一化；
  - 意图、时间、指标、维度、地区、排序、数量和关键词抽取；
  - 最长匹配消除“病例数/例数”等重叠指标；
  - 每轮清空上一轮检索、SQL、执行和图表状态；
  - 通过 custom stream 输出结构化实体标签。
- 新增 `DataAgentOrchestrationMiddleware`：
  - 强制 TableRAG -> SQL 校验 -> SQL 执行 -> ChartSpec 顺序；
...
- `uv run pytest tests\service_agent\test-data-agent -q`：64 passed。
