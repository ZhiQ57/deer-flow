...
- `agents/middlewares`：独立放置 `DataAgentTurnResetMiddleware` 与 `DataAgentOrchestrationMiddleware`，共用消息函数放在私有模块。
- `agents/thread_state.py`：定义 `DataAgentState`、结构化状态类型和 reducer。
- `deerflow/tools/builtins/entity_extract_tool.py`：提供 SDK 标准实体抽取 Tool、结构化类型和核心抽取器。
- `deerflow-dev/tools/builtins`：注册 SDK 实体工具，并按工具拆分 SQL 校验、SQL 执行和 ChartSpec 工具。
...
- QueryContext 从 `deerflow.tools.builtins.entity_extract_tool` 导入；SQL、数据库和图表基础能力分别从 `tools.sql_validation`、`tools.database`、`tools.chart_spec` 导入。
- 已删除 `agents.data_agent.middleware/state/tools/database/sql_validation/chart`，没有兼容层。
- 已删除 `agents.middlewares.query_context_middleware`、`tools.query_context` 和 `tools.builtins.query_context_tool`；实体抽取改为稳定 SDK `entity_extract_tool`，不保留旧导入兼容层。
