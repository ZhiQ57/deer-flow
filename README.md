...
2. **Run the setup wizard**
   ...
   models:
     ...
     - name: qwen3.6-plus
       ...
       use: deerflow.models.qwen_provider:QwenChatModel
       model: qwen3.6-plus
...
Servers that already expose globally unique names can set `tool_name_prefix: false`; the
DataAgent TableRAG entry uses this so its only visible MCP tool is exactly
`sqlrag_retrieve`.
...
Custom agents may also declare a versioned `service_ability`. The `data_query` v1 ability keeps the normal `lead_agent` route while dynamically adding TableRAG-backed intent labels, optional human confirmation, and a server-allowlisted PostgreSQL/MySQL SQL subagent. SQL validation, DSN resolution, database drivers, read-only transactions, result budgets, and error classification live only in the Gateway module `app.gateway.modules.sql_execution`; the publishable `deerflow-harness` package retains agent orchestration but does not connect to the business database or hold SQL execution secrets. The SQL subagent receives Gateway-provided tools whose implementations call trusted internal Gateway routes and return authoritative ToolMessage artifacts; failed executions require a new validation call before retrying within `sql_execution.max_execution_attempts`. When SQL execution is enabled, completed SQL code blocks in that custom-agent chat also expose an Execute action that calls the separate authenticated thread-scoped manual Gateway endpoint and displays the unmodified safe JSON result or database error in the right panel. The panel lets users select SQL, error, or result text and add it to the next conversation as a Sidecar reference. The manual UI path uses the same Gateway service and AST, read-only, dialect, Schema, binding, timeout, and result-budget enforcement without fabricating a Query Snapshot. Static table/column configuration allowlists are not part of the SQL Execution contract; future per-user permission checks belong in the Gateway authorization layer. It requires the custom agent to allowlist the SQL subagent, keeps internal SQL tools scoped to the approved snapshot and run capability, rejects stale state transitions, and supports same-target bindings plus explicit logical bindings such as a PostgreSQL TableRAG index mapped to a MySQL business source. See `docs/agents/data-agent/README.md` for the configuration and security boundaries.
