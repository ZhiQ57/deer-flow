...
Custom agents may also declare a versioned `service_ability`. The `data_query` v1 ability keeps the normal `lead_agent` route while dynamically adding TableRAG-backed intent labels, optional human confirmation, and a server-allowlisted read-only PostgreSQL/MySQL SQL subagent. SQL validation and database execution share `SqlExecutionService`; failed executions return bounded structured repair hints, and the SQL subagent may revalidate a repaired query and retry within `sql_execution.max_execution_attempts`. When SQL execution is enabled, completed SQL code blocks in that custom-agent chat also expose an Execute action that calls the authenticated thread-scoped Gateway SQL endpoint and displays the unmodified safe JSON result in the right panel. The manual UI path uses the same read-only, dialect, binding, allowlist, timeout, and result-budget enforcement as the subagent path, without fabricating a Query Snapshot. It requires the custom agent to allowlist the SQL subagent, keeps subagent SQL tools scoped to the approved snapshot, rejects stale state transitions, and supports same-target bindings plus explicit logical bindings such as a PostgreSQL TableRAG index mapped to a MySQL business source. See `docs/agents/data-agent/README.md` for the configuration and security boundaries.
