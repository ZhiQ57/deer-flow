...
Custom agents may also declare a versioned `service_ability`. The `data_query` v1 ability keeps the normal `lead_agent` route while dynamically adding TableRAG-backed intent labels, optional human confirmation, and a server-allowlisted read-only PostgreSQL/MySQL SQL subagent. SQL validation and database execution share `SqlExecutionService`; failed executions return bounded structured repair hints and a minimally sanitized database primary error, and the SQL subagent may revalidate a repaired query and retry within `sql_execution.max_execution_attempts`. When SQL execution is enabled, completed SQL code blocks in that custom-agent chat also expose an Execute action that calls the authenticated thread-scoped Gateway SQL endpoint and displays the unmodified safe JSON result or database error in the right panel. The panel lets users select SQL, error, or result text and add it to the next conversation as a Sidecar reference. The manual UI path uses the same AST, read-only, dialect, Schema, binding, timeout, and result-budget enforcement as the subagent path, without fabricating a Query Snapshot. Static table/column configuration allowlists are not part of the SQL Execution contract; future per-user permission checks belong in the Executor authorization layer. It requires the custom agent to allowlist the SQL subagent, keeps subagent SQL tools scoped to the approved snapshot, rejects stale state transitions, and supports same-target bindings plus explicit logical bindings such as a PostgreSQL TableRAG index mapped to a MySQL business source. See `docs/agents/data-agent/README.md` for the configuration and security boundaries.
