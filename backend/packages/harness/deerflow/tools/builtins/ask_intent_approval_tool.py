...
def ask_intent_approval_tool(
    ...
    asklist列表是集合对象, 包含两个字段, 负责承载多个问题的集合:
    - question: 问题文本,对有歧义的意图, 向用户提问.
    - options: 问题对应的审批选项列表, 例如: ["确认", "取消", "..."], 请按照实际选项填写, 如果没有选项可使用输入文本框获取用户输入.

    一个问题只能对应一个审批选项列表, 如果有多个问题, 请在 asklist 中添加多个 AskQuestionList 对象.
    例如:
    [
        {"question": "时间xx?", "options": ["确认", "需要调整"]},
        {"question": "性别?", "options": ["男性", "女性"]},
        {"question": "数值?", "options": ["100", "200", "需要调整"]},
        {"question": "统计口径?", "options": ["聚合统计", "需要调整"]}
    ]
