...
DataAgent production queries use the generic `AgentConfig.service_ability` extension. `data_query` v1 requires `enable_sql_rag=true`, is resolved under `deerflow.agents.service_agent`, injects tools and middleware at the existing lead-agent extension anchor, and persists authorization only in `ThreadState.service_states`; remove the whole ability to disable it rather than creating a label-only half-profile. The formal SQL path supports PostgreSQL/MySQL. `agents/service_agent/sql_executor.py` owns the shared `SqlExecutionService`, typed validation/execution requests and results, AST validation, database drivers, result budgets, binding checks, and safe database-error classification; `sql_tools.py` only owns the snapshot-scoped SQL SubAgent tool session, and its `data_validate_sql` / `data_execute_sql` results must use `content_and_artifact` so ToolMessage artifact is the parent reconstruction authority while content remains model-visible fallback.

`source=subagent` requires Query Snapshot and TableRAG registry; `source=manual_ui` resolves the current binding on the server and keeps AST/read-only/dialect/Schema/binding checks without fabricating a Snapshot. SQL Execution configuration does not contain static table/column allowlists; future per-user permissions belong in an Executor authorization layer. The two sources are included in the validation digest and cannot reuse each other's approval. Database-driver failures preserve a minimally sanitized primary error for the frontend and SQL SubAgent. A failed SubAgent execution consumes the current validation generation, so every repair or retry must call `data_validate_sql` again; the session permits at most `sql_execution.max_execution_attempts` database calls (default 3, range 1-5) and stops permanently after success. Repairable driver failures expose bounded `error_category`, optional sanitized `error_message`, `retryable`, and `recommended_action`, never DSNs or tracebacks.

TableRAG retrieval is exposed only as the exact tool name `sqlrag_retrieve`; configure its MCP Server with `tool_name_prefix=false`, and use the six-value `operation` contract rather than legacy per-route tools. `same_physical_target` requires equal retrieval/execution fingerprints; `logical_data_source` supports an explicitly configured PostgreSQL TableRAG index mapped to a MySQL business source and binds both targets into one server-generated digest. The custom agent must explicitly list the configured SQL subagent in `allowable_subagents`, and the subagent config must separately allow only `data_validate_sql` / `data_execute_sql`. `sql_only` never receives the execution tool, and parent state is projected only from captured SQL tool messages with artifact-first parsing, not subagent free text.
...
| **SQL Execution** (`/api/threads/{id}/sql/execute`) | `POST /` - execute a completed DataAgent SQL code block with `source=manual_ui`. Requires exact thread ownership and a current-user custom-agent with enabled `data_query/sql_execution`; reuses `SqlExecutionService.aexecute()` so synchronous database drivers stay off the ASGI event loop. SQL-level validation failures include the rejected Schema or structural rule, and driver failures include a minimally sanitized primary error; neither response exposes credentials or tracebacks |
