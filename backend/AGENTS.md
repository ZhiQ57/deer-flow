...
**Experimental DataAgent** (`packages/harness/deerflow-dev/`):
- Mirrors the SDK module boundaries instead of keeping all business code under one agent directory: graph assembly and prompts live in `agents/data_agent/`, DataAgent middleware in `agents/middlewares/`, state in `agents/thread_state.py`, callable tools in `tools/builtins/`, tool implementation helpers in `tools/`, and future built-in vertical subagents in `subagents/builtins/`
- Console/prototype entry points: `from agents import build_data_agent, make_data_agent`; old `agents.data_agent.middleware/state/tools/database/sql_validation/chart` imports are removed without compatibility shims
- Uses `deerflow.agents.factory.create_deerflow_agent(...)` to recreate a lead-agent-like graph while injecting `QueryContextMiddleware` and `DataAgentOrchestrationMiddleware`
- Test execution script: `tests/service_agent/test-data-agent/run_data_agent_stream.py`; it streams `values`, `messages`, and `custom` events directly in the console and does not add a Gateway route
- The runner accepts `--log-path` as a directory or `log.txt` template, creates `log_YYYYMMDD_HHMMSS_mmm.txt`, records runtime arguments/environment/path checks plus stream/tool/stage output, captures Python root logs, and redacts DSN passwords and secret-like environment variables
- Local structured debug page: `tests/service_agent/test-data-agent/run_data_agent_web.py`; it serves a loopback-only FastAPI/NDJSON browser UI with chat, stage timeline, QueryContext, TableRAG retrieval, SQL validation/execution tables, ChartSpec preview, tool events, and per-run timestamped logs. It uses an in-memory checkpointer for same-page conversation continuity, allows one active run, and is not a production Gateway route
- Depends on the single unprefixed TableRAG MCP tool `sqlrag_retrieve` from `packages/harness/table_rag` and the public `table-rag-agent` skill in `skills/public/z_sqltable-rag`; `operation` selects `hybrid-search`, four `search-*` routes, or `expand-join-graph`
- Uses a fail-closed tool surface: required framework read tools, exactly `sqlrag_retrieve`, and `data_validate_sql` / `data_execute_sql` / `data_build_chart_spec`; no shell, file writes, unrelated MCP, legacy TableRAG tool names, or index mutations
- SQL execution is MySQL-only, single-statement `SELECT/WITH`, bound to the most recent validation digest, database-scoped, timeout/row/result-budgeted, and wrapped in a read-only transaction; production deployments must still use a database-level read-only account
- State includes query/retrieval/validation/execution/chart stages plus `data_last_successful_sql_execution`, so a failed replacement query does not erase the last usable result
- Default per-turn budgets are 6 TableRAG retrievals, 4 validations, 2 executions, and 2 chart builds; reaching the execution budget also blocks further retrieval/validation so structured results cannot be overwritten
- The native custom-agent UI path remains the regular lead-agent graph; only the direct experimental entry points have these DataAgent-specific middleware and runtime tools
...
DataAgent production queries use the generic `AgentConfig.service_ability` extension. `data_query` v1 requires `enable_sql_rag=true`, is resolved under `deerflow.agents.service_agent`, injects tools and middleware at the existing lead-agent extension anchor, and persists authorization only in `ThreadState.service_states`; remove the whole ability to disable it rather than creating a label-only half-profile. The formal SQL path supports PostgreSQL/MySQL, but its application boundary is Gateway-only: `app/gateway/modules/sql_execution/` owns request/response contracts, DSN and binding resolution, sqlglot validation, database drivers, read-only transactions, result budgets, safe error classification, run capabilities, routes, and the Gateway SQL SubAgent tool provider. `deerflow-harness` must not import this module, resolve SQL DSNs, depend on SQL drivers/sqlglot, or execute business SQL. Harness exposes only the generic `deerflow.subagents.tool_provider` extension point; Gateway registers a provider that gives the approved SQL SubAgent `data_validate_sql` / `data_execute_sql` tools whose coroutine implementations call `/api/internal/threads/{id}/sql/{validate,execute}` with trusted internal authentication. Those routes reload the current owner, custom agent, checkpoint, approved Snapshot and retrieval binding, while request bodies accept only candidate SQL plus run/snapshot identity and validation digest—never DSN, Secret, Schema, or registry. Gateway injects only a no-secret `data_query_binding` into Harness runtime context and stores request-scoped database Secrets in the process-local run capability registry, and its `data_validate_sql` / `data_execute_sql` results must use `content_and_artifact` so ToolMessage artifact is the parent reconstruction authority while content remains model-visible fallback.

`source=subagent` requires the approved Query Snapshot and TableRAG registry; `source=manual_ui` resolves the current binding on the server and keeps AST/read-only/dialect/Schema/binding checks without fabricating a Snapshot. The sources are included in the validation digest and cannot reuse each other's approval. A failed SubAgent execution consumes the current validation generation, so every repair or retry must explicitly call `data_validate_sql` again; the Gateway registry permits at most `sql_execution.max_execution_attempts` calls (default 3, range 1-5) and stops permanently after success. Database-driver failures expose bounded `error_category`, optional minimally sanitized `error_message`, `retryable`, and `recommended_action`, never DSNs or tracebacks.

TableRAG retrieval is exposed only as exact tool name `sqlrag_retrieve`; configure its MCP Server with `tool_name_prefix=false`, and use the six-value `operation` contract rather than legacy per-route tools. `same_physical_target` requires equal retrieval/execution fingerprints; `logical_data_source` supports an explicitly configured PostgreSQL TableRAG index mapped to a MySQL business source and binds both targets into one server-generated digest. The custom agent must explicitly list the configured SQL subagent in `allowable_subagents`, and the subagent config must separately allow only `data_validate_sql` / `data_execute_sql`. `sql_only` never receives the execution tool, and parent state is projected only from authoritative Gateway ToolMessage artifacts with artifact-first parsing, not subagent free text.

The service reducer rejects stage rollback and stale snapshot writes while allowing a new visible user query to replace old state by writing `retrieving` / `needs_refinement` directly from TableRAG, or by writing `labels_published` with a matching `payload.resumed_from` when the model deliberately reuses the current historical retrieval snapshot. Production DataAgent no longer registers a turn-reset middleware. `publish_query_labels` only publishes a snapshot plus `approval_policy`; it may republish labels from a `cancelled` / `succeeded` / `failed` snapshot when that snapshot still carries valid retrieval context, so the model can reopen the label/approval flow after reading history. When approval is needed the model must call DataAgent-only `ask_intent_approval`, whose human-input result is written back as a real ToolMessage so later turns can read the historical human decision. SQL stage uses the current persisted snapshot only: approved or policy-approved can enter SQL, while `cancelled` cannot be revived into SQL without publishing a new label snapshot and receiving approval again.

If the SQL stage rejects a task call, its `ToolMessage` must carry terminal `subagent_status=failed` metadata; delegation capture also recognizes historical `SQL_*` stage-error JSON and settles old `in_progress` entries with a previous `run_id` when a new run starts, so stale checkpoints cannot keep model context or UI cards in `in_progress`. Per-model-response TableRAG, label, approval, and SQL task calls are serialized to one. DataAgent `ambiguities` must be explicitly declared; omitted ambiguity fields fail closed and create a review item. ToolMessage artifact must survive both `messages-tuple` and `values` serialization so the frontend can render the intent/review card, SQL result card, and human-input state; `task` SQL results also carry `data_query_sql_result` artifact so long content previews cannot break the parent contract. Do not add DataAgent-specific graphs, top-level state keys, globally registered SQL/label tools, or another database execution implementation. The public DataAgent-specific REST surface remains the authenticated manual UI endpoint below; the internal SQL routes are Gateway-only implementation details and are not frontend or SDK APIs.
...
34. **ClarificationMiddleware** - Intercepts `ask_clarification` tool calls, writes a readable `ToolMessage.content` fallback plus structured `ToolMessage.artifact.human_input` request payload, and interrupts via `Command(goto=END)` (must be last). Because human-input middlewares can short-circuit tool execution before LangChain emits `on_tool_end`, `RunJournal` performs a root-run final reconciliation for allowlisted human-input ToolMessages (`ask_clarification` and DataAgent `ask_intent_approval`) whose `tool_call_id` was produced by the current run, so request cards remain recoverable from `run_events` after checkpoint compaction. Human Input Card replies are submitted as `hide_from_ui` `HumanMessage`s with `additional_kwargs.human_input_response`; `RunJournal` persists only allowlisted hidden response sources (currently `ask_clarification` and `ask_intent_approval`) as `llm.human.input`, which preserves answered-card state after compaction without exposing generic internal hidden context.
...
| **SQL Execution** (`/api/threads/{id}/sql/execute`, `/api/internal/threads/{id}/sql/{validate,execute}`) | The public `POST /api/threads/{id}/sql/execute` executes a completed DataAgent SQL code block with `source=manual_ui`, exact thread ownership, and a current-user custom-agent with enabled `data_query/sql_execution`. The internal validate/execute routes accept only Gateway-authenticated SQL SubAgent calls, re-read the approved Snapshot and binding, enforce per-run validation generations and attempt budgets, and return authoritative artifacts. All routes reuse Gateway `SqlExecutionService`; synchronous drivers run through `aexecute()` off the ASGI event loop. No response exposes credentials or tracebacks |
