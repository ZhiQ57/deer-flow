...
DataAgent production queries use the generic `AgentConfig.service_ability` extension. `data_query` v1 requires `enable_sql_rag=true`, is resolved under `deerflow.agents.service_agent`, injects tools and middleware at the existing lead-agent extension anchor, and keeps approval/SQL linkage in `ToolMessage` history via `flow_id`; remove the whole ability to disable it rather than creating a label-only half-profile. The formal SQL path supports PostgreSQL/MySQL, but its application boundary is Gateway-only: `app/gateway/modules/sql_execution/` owns request/response contracts, DSN and binding resolution, sqlglot validation, database drivers, read-only transactions, result budgets, safe error classification, run capabilities, routes, and the Gateway SQL SubAgent tool provider. `deerflow-harness` must not import this module, resolve SQL DSNs, depend on SQL drivers/sqlglot, or execute business SQL. Harness exposes only the generic `deerflow.subagents.tool_provider` extension point; Gateway registers a provider that gives the approved SQL SubAgent `data_validate_sql` / `data_execute_sql` tools whose coroutine implementations call `/api/internal/threads/{id}/sql/{validate,execute}` with trusted internal authentication. Those routes reload the current owner, custom agent, flow and Gateway-injected binding, while request bodies accept only candidate SQL plus run/flow identity and validation digest - never DSN, Secret, Schema, retrieval state, or registry. Gateway injects only a no-secret `data_query_binding` into Harness runtime context and stores request-scoped database Secrets in the process-local run capability registry, and its `data_validate_sql` / `data_execute_sql` results must use `content_and_artifact` so ToolMessage artifact is the parent reconstruction authority while content remains model-visible fallback.

`source=subagent` requires the approved Query Flow and the Gateway-injected binding; `source=manual_ui` resolves the current binding on the server and keeps AST/read-only/dialect/Schema/binding checks without fabricating a Flow. The sources are included in the validation digest and cannot reuse each other's approval. A failed SubAgent execution consumes the current validation generation, so every repair or retry must explicitly call `data_validate_sql` again; the Gateway registry permits at most `sql_execution.max_execution_attempts` calls (default 3, range 1-5) and stops permanently after success. Database-driver failures expose bounded `error_category`, optional minimally sanitized `error_message`, `retryable`, and `recommended_action`, never DSNs or tracebacks.
