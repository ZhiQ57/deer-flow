...
- Depends on the TableRAG MCP tools from `packages/harness/table_rag` and the public `table-rag-agent` skill in `skills/public/z_sqltable-rag`
- Uses a fail-closed tool surface: required framework read tools, read-only TableRAG MCP tools, and `data_validate_sql` / `data_execute_sql` / `data_build_chart_spec`; no shell, file writes, unrelated MCP, or TableRAG index mutations
